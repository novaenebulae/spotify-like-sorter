...
class PreviewManager:
    ...
    async def fetch_preview(
        ...
        try:
            async with DeezerClient() as deezer:
                ...
                if result:
                    logger.info(f"✅ Deezer trouvé: {track.get('isrc')} → {result['preview_url'][:60]}...")
                    return {
        ...
        try:
            async with iTunesClient() as itunes:
                ...
                if result:
                    logger.info(f"✅ iTunes trouvé: {track.get('isrc')} → {result['preview_url'][:60]}...")
                    return {
                        'preview_url': result['preview_url'],
                        'source': 'itunes',
                        'fetched_at': datetime.utcnow()
        ...
        # 3. Pas trouvé
        logger.warning(f"⚠️ No preview found: {track_id}")
        return None
    ...
    async def process_all_tracks(
        ...
        logger.info(f"✅ Phase 1 complete: {stats['fetched']} found, {stats['not_found']} not found")
        ...
        if download and download_queue:
            ...
            logger.info(f"✅ Phase 2 complete: {stats['downloaded']} downloaded")
